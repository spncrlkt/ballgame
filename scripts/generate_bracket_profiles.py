...
Always produces exactly 64 profiles:
- Top N performers from previous bracket (protected seeds, configurable via --top-seeds)
- Remaining slots filled with extreme variant profiles generated from top performers

Variants are generated by applying archetype mutations (Aggro, Zone, Sniper, etc.)
to the top performing profiles, creating extreme playstyle specializations.
"""
...
# Total profiles to generate (standard bracket size)
TOTAL_PROFILES = 64
...
Output is always exactly 64 profiles for bracket tournaments.
        """
    )
    parser.add_argument(
        "--db", required=True,
        help="Path to SQLite database with bracket results (REQUIRED)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to profiles file used in tournament (REQUIRED)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Number of top performers to preserve as seeds (default: 16)"
    )
    parser.add_argument(
        "--variant-bases", type=int, default=8,
        help="Number of top profiles to use as bases for variant generation (default: 8)"
    )
    parser.add_argument(
        "--version", default="v13",
    ...
    # Calculate how many variants we need for exactly 64 total
    variants_needed = TOTAL_PROFILES - args.top_seeds

    # Load top performers from bracket - request more than we need for variant bases
    request_count = max(args.top_seeds, args.variant_bases) + 8
    ...
    # Use top N for variant generation (configurable via --variant-bases)
    variant_base_count = min(args.variant_bases, len(top_profiles))
    variant_bases = top_profiles[:variant_base_count]

    # Warmup seed: first unprotected profile (rank top_seeds + 1)
    # This is the highest-ranked profile that will be replaced by variants
    warmup_rank = actual_top_seeds  # 0-indexed, so this is rank (top_seeds + 1)
    if warmup_rank < len(top_profiles):
        warmup_seed_name = top_profiles[warmup_rank].name
    else:
        # Fallback: use last available profile
        warmup_seed_name = top_profiles[-1].name
    ...
    # Generate variants: cycle through archetypes × base profiles
    print(f"\nGenerating {variants_needed} extreme variants from {len(variant_bases)} bases...")
    variants = []
    ...
    # Generate variants (may overshoot slightly due to archetype cycling)
    while len(variants) < variants_needed:
    ...
    # Trim extras randomly to hit exactly the target count
    if len(variants) > variants_needed:
        random.shuffle(variants)
        variants = variants[:variants_needed]
