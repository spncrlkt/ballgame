...
- Training debug analysis: `cargo run --bin analyze -- --training-db $(ls -t db/training_*.db | head -1)` writes reports + derived grids to `training_logs/session_<timestamp>/analysis`.
